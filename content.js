(() => {
    // ================================= BLOQUEIA ENTER PARA NÃO DEIXAR INSERIR PACIENTE NO MOMENTO DA PESQUISA USANDO ESSA TECLA ================================
    document.addEventListener("keydown", function (e) {
        ...
        const dropdown = input.closest(".select2-dropdown");
        if (!dropdown) return;

        if (!dropdown.querySelector("#select2-PacienteID-results")) return;

        const semResultado = dropdown.querySelector(".btn-inserir-si");
    ...
    // ================================= BLOQUEIA O CHECKBOX "SEM CPF" NA TELA DE AGENDAMENTO ================================
    function bloquearSemCPF() {

        //  DETECTOR DA TELA DE AGENDAMENTO
        const temEncaixe = document.getElementById("Encaixe");
        const temRetorno = document.getElementById("Retorno");

        if (!temEncaixe || !temRetorno) return;

        const semCpf = document.getElementById("SemCPF-ageCPF");
        if (!semCpf) return;

        semCpf.checked = false;

        // impede clique
        semCpf.addEventListener("click", function (e) {
            e.preventDefault();
            e.stopPropagation();
        }, true);

        // impede mudança via script
        semCpf.addEventListener("change", function (e) {
            e.preventDefault();
            e.stopPropagation();
            semCpf.checked = false;
        }, true);
